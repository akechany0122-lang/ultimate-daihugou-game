  ...
  const handleStartGameTransition = () => {
    ...
    preloadImages(['room home.png', 'online.png', 'CPU.png'], () => {
      ...
      const executePass = (playerIndex) => {
        ...
        const isServiceAce = tableCards.length > 0 && tableCards.every(tc => tc.card.rank === 'A');
        const requiredPassCount = isServiceAce ? activePlayersCount : (activePlayersCount - 1);
      ...
      const evaluateSpecialEffects = (playerIndex, playedCards, currentHands, currentGraveyard) => {
        ...
        const effectDelay = isPlus5Combo ? 6500 : 3400;
        ...
        return { h, gy, shieldStates, is11B, isRev, isFieldCleared, steps: kWait ? 0 : steps, pDir, isAsync, phoenixDrawn };
      };
      ...
      const executePlay = (playerIndex, cards) => {
        ...
        const phoenixTotalTime = effectRes.phoenixDrawn
          ...
          : 0;
        let totalAnimTime = isPlus5Combo ? 9000 : (phoenixTotalTime > 0 ? phoenixTotalTime : (effectRes.isAsync ? (playRank === 'Q' ? 3500 : (playRank === 'K' ? 5000 : 3000)) : (isEffectSelecting && playerIndex === 0 ? 4000 : 2800)));
          ...
          >
            ...
            {playerIdx !== myPlayerIndex ? (
              <div className="player-center-info" style={{ filter: passedPlayers.includes(playerIdx) ? 'brightness(0.3)' : 'none' }}>
                ...
                {/* アバターアイコン */}
                <div className={`player-avatar-icon ${hierarchy ? hierarchy.cls : ''}`} style={getAvatarStyle(getPlayerAvatar(playerIdx))}></div>
                {/* 名前はアイコンの下部に配置 */}
      ...
      // 選択モード用 10秒タイマー
      useEffect(() => {
        if (!selectionMode && !watchSelecting) {
        ...
        return () => clearTimeout(timer);
      }, [selectionMode, watchSelecting, selectionTimeLeft, turn]);
...
{multiplayerMode === 'select' && (
            ...
            <div className="table-container">
              ...
              <div className="player-you-info-fixed">
                ...
                <div className={`player-avatar-icon ${ranksDiscovered.includes(myPlayerIndex) ? getHierarchyTitle(ranksDiscovered.indexOf(myPlayerIndex) + 1).cls : ''}`} style={getAvatarStyle(getPlayerAvatar(myPlayerIndex))}></div>
                <div className="player-status">
                  YOU
              ...
              <button className="btn-exit-to-title mystic-btn" onClick={() => {
                sm.playPaperSE();
                if (roomChannel) {
                ...
                setMultiplayerMode(null);
                setTitleStage(2);
              }}><span className="mystic-btn-text">退場</span></button>
              {multiplayerMode === 'lobby' && (
          ...
          {abilityLog && <div className="ability-log-overlay">{abilityLog}</div>}
                ...
                return (
                  <div key={pIdx} className={`player-pos ${posClass} ${isActive ? 'active-turn' : ''} ${watchClasses}`} style={targetStyle}>
                    ...
                    {ranksDiscovered.includes(pIdx) ? (
                      <div className={`finished-aura-container ${getHierarchyTitle(ranksDiscovered.indexOf(pIdx) + 1).cls}`}>
                        <div className="finished-aura-text">{getHierarchyTitle(ranksDiscovered.indexOf(pIdx) + 1).title}</div>
                      </div>
                    ) : (
              ...
              {(() => {
                ...
                if (allHumansFinished) {
                  return (
                    <div style={{ position: 'absolute', bottom: '200px', left: '50%', transform: 'translateX(-50%)', zIndex: 1000 }}>
                      <button className={`action-btn ${isFastForward ? 'btn-pass' : 'btn-confirm'} mystic-btn`} onClick={() => {
                        setIsFastForward(!isFastForward);
                        if (multiplayerMode === 'active' && !isHost) {
                          sendActionToHost({ action: 'FAST_FORWARD' });
                        }
                      }}>
                        <span className="mystic-btn-text">▶▶ 早送り {isFastForward ? 'ON' : 'OFF'}</span>
                      </button>
                    </div>
                  );
                }
